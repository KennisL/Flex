...
{
    Q_GADGET			//该类有Q-Object 功能
    Q_ENUMS(DockArea)	//添加枚举的映射
    Q_ENUMS(DockMode)
    Q_ENUMS(ViewMode)
public:
    enum DockArea
...
public:
    ...
    static QWidget* window(); //返回widget指针
    ...
    static inline QWidget* parent(ViewMode viewMode, QWidget* widget)	
    {
        if (viewMode == Flex::ToolView || viewMode == Flex::ToolPanesView || viewMode == Flex::ToolPagesView)
        {
            return widget->parentWidget() ? widget->parentWidget() : Flex::window();		//返回父窗体指针，没有就返回自身的指针
        }
        else
        {
            return nullptr;
